/**
 * Shared consensus logic - fetches from Binance, CoinGecko, CoinPaprika, CryptoCompare
...
/** Fetch consensus prices directly - no HTTP self-call. Use this from API routes. */
export async function getConsensusPrices(currency: string): Promise<ConsensusResult> {
  const curr = (currency || "usd").toLowerCase();
  const rate = FIAT_TO_USD[curr] ?? 1;

  const [binance, coingecko, coinpaprika, cryptocompare] = await Promise.all([
    fetchBinance(curr),
    fetchCoinGecko(curr),
    fetchCoinPaprika(curr),
    fetchCryptoCompare(curr),
  ]);
